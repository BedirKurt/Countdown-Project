...
<template>
    <div class="min-h-screen bg-stars bg-cover bg-no-repeat overflow-hidden">
        ...
        <main>
            ...
            <div class="relative flex justify-center items-center pt-6 h-12">
                <button @click="startCountdown" :disabled="isRunning"
                    ...
                    :class="isRunning ? 'left-[calc(40%-9rem)]' : 'left-1/2 -translate-x-1/2'">
                    <BasePlayButton />
                </button>
                <transition name="fade">
                    <button v-if="isRunning" @click="pauseCountdown"
                        class="absolute left-1/2 -translate-x-1/2 transition-opacity duration-500 button w-32 h-7 p-6 flex justify-center items-center bg-[#ff284818] rounded-2xl border-none cursor-pointer hover:bg-[#ff284839]">
                        <BasePauseButton />
                    </button>
                </transition>
                <transition name="fade">
                    <button v-if="isRunning" @click="resetCountdown"
                        class="absolute transition-opacity duration-500 button w-28 h-7 p-4 flex justify-center items-center bg-[#ff284818] rounded-2xl border-none cursor-pointer hover:bg-[#ff284839] custom-button-position ">
                        <BaseResetButton />
                    </button>
                </transition>
            </div>
            <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div class="bg-countdownP rounded-2xl p-6 shadow-lg max-w-56  md:max-w-96 w-full text-center">
                    <h2 class="text-xl font-semibold text- mb-2 text-[#FF2849]">Warning</h2>
                    <p class="text-white mb-4 font-redhatFont">
                        Please enter an hour, minute, or second value to start the timer.
                    </p>
                    <button @click="showModal = false"
                        class="mt-2 px-4 font-redhatFont bg-gray-500 hover:bg-countdownP text-[#FF2849] rounded-lg transition">
                        OK
                    </button>
                </div>
            </div>
